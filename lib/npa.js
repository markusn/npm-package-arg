...
function isRegistrySpec (spec) {
  ...
  return spec.toLowerCase().startsWith('registry:')
}
...
function resolve (name, spec, where, arg) {
  ...
  if (isFileSpec(spec)) {
    ...
    return fromRegistrySpec(res, where)
  }
...
function toPurl (arg, reg = defaultRegistry) {
  ...
  // Always respect the registryUrl if it's set, otherwise use the provided registry
  const registry = res.registryUrl || reg || defaultRegistry
...
class Result {
  constructor (opts) {
    ...
    this.registryUrl = this.registry && (opts.registryUrl || defaultRegistry)
  }
...
function fromRegistrySpec (res, where) {
  const match = res.rawSpec.match(/^registry:([^#]+)(?:#(.+))?$/)
  const subSpec = npa(match[2], where)

  subSpec.raw = res.raw
  subSpec.registryUrl = match[1]
  return subSpec
}
